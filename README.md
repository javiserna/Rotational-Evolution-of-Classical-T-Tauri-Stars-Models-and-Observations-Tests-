...
In the next plots, we show the obtained distributions of vsin(i) provided by models in comparison with our observations. We use the histogram intersection algorithm to get the intercepted area between histograms, we use this value as a metric of similarity between distributions. If the metric is 1 means both distributions are the same, if the metric is zero means there is no correlation.
